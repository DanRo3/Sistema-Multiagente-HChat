...
def test_query(
    ...
    try:
        ...
        if not final_results:
            ...
            return
        for i, (doc, score) in enumerate(final_results[:k]): # Muestra hasta k resultados finales
...
if __name__ == "__main__":
    ...
    if not embeddings_model:
        ...
        if not faiss_db:
            ...
            # --- Parámetros de la Prueba 2: Búsqueda Genérica + Filtro ---
            test_query_text: str = "capitán"    # <-- CONSULTA GENÉRICA
            number_of_results: int = 20       # <-- k MÁS ALTO para dar chance al filtro
            search_method: str = "similarity"
            # <-- APLICA EL FILTRO POR METADATOS
            metadata_filter: Optional[Dict[str, Any]] = {"master_name": "Litlejohn"}
            # -----------------------------------------------------------

            print("\n*** EJECUTANDO PRUEBA 2: BÚSQUEDA GENÉRICA + FILTRO ***")
            test_query(
                vector_store=faiss_db,
                query=test_query_text,
                k=number_of_results, # Pasar k más alto a la búsqueda inicial
                search_type=search_method,
                filter_metadata=metadata_filter
            )
            # Nota: La función test_query internamente mostrará como máximo number_of_results
